# step7_node2vec_clustering.py  (pure node2vec + silhouette-k)
...
def node2vec_walk(G, start, walk_length=40, p=1.0, q=1.0):
    walk = [start]
    ...
    for _ in range(walk_length - 2):
        ...
        for nxt in curr_neighbors:
            ...
            w = G[curr][nxt].get("weight", 1.0)
            weights.append(bias * w)

        total = sum(weights)
        probs = [w / total for w in weights]
        nxt = random.choices(curr_neighbors, probs)[0]
        walk.append(nxt)

    return walk


def generate_node2vec_walks(G, num_walks=80, walk_length=40, p=1.0, q=1.0, seed=42):
    random.seed(seed)
    nodes = list(G.nodes())
    walks = []
    for _ in range(num_walks):
        ...
        for n in nodes:
            w = node2vec_walk(G, n, walk_length=walk_length, p=p, q=q)
...
def choose_best_k_silhouette(embeddings, year, algo_name, out_dir,
    ...
    """
    Returns best_k and list of (k, score).
    Saves silhouette curve plot per year.
    """
    n = embeddings.shape[0]
    if n < 3:
        # can't compute silhouette properly
        return max(1, n), []
    ...
    for k in k_candidates:
        ...
        # silhouette requires >=2 clusters and not all points in one cluster
        if len(set(labels)) < 2:
            continue
...
def run_node2vec_clustering(G, year, dim=128, walk_length=40, num_walks=80,
                            p=1.0, q=1.0, seed=42,
                            silhouette_dir=None):
    nodes = list(G.nodes())
    if len(nodes) == 0:
    ...
    best_k, _ = choose_best_k_silhouette(
        emb, year, "node2vec", silhouette_dir, k_min=2, k_max=50, seed=seed
    )

    print(f"  Node2Vec ({year}): {len(nodes)} nodes, silhouette-best k={best_k}")
...
def compute_temporal_stability(cluster_csv_path, stability_out_path, jaccard_threshold=0.0):
    ...
    for i in range(len(years) - 1):
        ...
        part_y, part_next = year_partitions[y], year_partitions[y_next]
...
if __name__ == "__main__":
    ...
    with open(cluster_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for year in sorted(year_graphs.keys()):
            print(f"Running Node2Vec for year {year}...")
            part = run_node2vec_clustering(
                year_graphs[year],
                year=year,
