...
def run_infomap(G):
    """
    Run Infomap community detection via cdlib.

    Works even if external `infomap` package is not installed:
    cdlib will fallback to igraph's infomap implementation.
    """
    # IMPORTANT: no weight kwarg in cdlib.infomap
    res = algorithms.infomap(G)
...
if __name__ == "__main__":
    ...
    with open(cluster_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for year in sorted(year_graphs.keys()):
            G = year_graphs[year]
            print(f"Running Infomap for year {year}...")
