...
PROMPT_TEMPLATE = ChatPromptTemplate.from_messages([
    (
        "system",
        """
You are an academic text-processing assistant specialized in cleaning, normalizing, and standardizing academic keywords.

Processing Rules:
1. Convert all keywords to lowercase.
2. Remove stopwords such as: "analysis", "study", "approach", "method", "paper", "system", "based" etc.
3. Remove punctuation and special characters such as "-","_"," !" etcs.
4. Expand academic acronyms (e.g., "nlp" → "natural language processing").
5. Apply lemmatization; if it fails, apply stemming.
6. Only return meaningful academic keywords.

Developer Instructions:
- Always return valid JSON with the exact field name: cleaned_keywords
- Never output additional text.
- Never wrap JSON in code blocks.
- Never hallucinate unknown acronyms.
- Ensure JSON only contains the required field.
- Ensure all cleaned keywords are strings.
...
# PATHS
# ============================
input_file = r"graphs using keyword processing\acm_cyber_security_regex.jsonl"
output_dir = r"keyword_processing"
os.makedirs(output_dir, exist_ok=True)
base_output = os.path.join(output_dir, "acm_cyber_security_keywords")
