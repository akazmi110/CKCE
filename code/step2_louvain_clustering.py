...
# ---------- Helpers to load graphs from edgelists ----------

def load_year_graphs_from_edgelists(edges_dir):
    """
    Load yearwise graphs from edge list CSV files.

    Expects files like: 2011_edgelist.csv, 2012_edgelist.csv, ...
    ...
    for csv_path in edges_dir.glob("*_edgelist.csv"):
        year_str = csv_path.stem.split("_")[0]  # e.g., '2011' from '2011_edgelist'
        try:
            year = int(year_str)
        except ValueError:
...
def build_clusters_by_id(partition):
    """
    partition: {node: cluster_id}
    returns: {cluster_id: set(nodes)}
    """
...
def compute_temporal_stability(cluster_csv_path, stability_out_path, jaccard_threshold=0.0):
    """
    Compute temporal stability using best Jaccard overlap between clusters
    in consecutive years.

    Input cluster CSV must have columns:
        year,keyword,cluster_id

    Output CSV columns:
        year, next_year, num_clusters_year, num_clusters_next,
        matched_clusters, mean_best_jaccard, median_best_jaccard, std_best_jaccard
    """
    ...
    # Load clusters per year
    year_partitions = defaultdict(dict)  # year -> {keyword: cluster_id}
...
def plot_temporal_stability(stability_csv_path, algo_name, out_dir):
    """
    Plot mean_best_jaccard over years for this algorithm.
    """
    ...
    with open(stability_csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            y = int(row["year"])
            mean_j = float(row["mean_best_jaccard"])
            years.append(y)
...
def plot_num_clusters_over_time(cluster_csv_path, algo_name, out_dir):
    """
    Plot number of clusters per year for this algorithm.
    """
    cluster_csv_path = Path(cluster_csv_path)
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # year -> set(cluster_ids)
...
if __name__ == "__main__":
    # Adjust these paths
    edges_dir = r"clustering paper\data\co_occurance_graphs"
    clusters_dir = r"clustering paper\data\clusters"
    algo_name = "louvain"
    ...
    # 2) Run Louvain per year and save clusters
    ...
    with open(cluster_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for year in sorted(year_graphs.keys()):
            G = year_graphs[year]
            print(f"Running Louvain for year {year}...")
            partition = run_louvain(G)  # {node: cluster_id}
            for kw, cid in partition.items():
                writer.writerow([year, kw, cid])

    print(f"Cluster assignments saved to: {cluster_csv_path}")

    # 3) Compute temporal stability and save
